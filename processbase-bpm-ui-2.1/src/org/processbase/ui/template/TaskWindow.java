...
//    protected Button applyBtn = new Button(messages.getString("btnOK"));
    ...
    public TaskWindow(String caption) {
        super(caption);
    ...
    public boolean setTaskInfo(String processDefinitionUUID) {
        try {
            if (processDefinitionUUID == null) {
                String taskUUID = (String) PbPortlet.getCurrent().portletSession.getAttribute("PROCESSBASE_SHARED_TASKINSTANCE", PortletSession.APPLICATION_SCOPE);
                task = taskUUID != null ? PbPortlet.getCurrent().bpmModule.getTaskInstance(new ActivityInstanceUUID(taskUUID)) : null;
            } else {
                processDefinition = PbPortlet.getCurrent().bpmModule.getProcessDefinition(new ProcessDefinitionUUID(processDefinitionUUID));
            }
            if (task == null & processDefinition == null) {
                ...
                if (!this.task.isTaskAssigned() || !this.task.getTaskUser().equals(((User)PbPortlet.getCurrent().getUser()).getScreenName())) {
                    throw new Exception("TASK NOT ASSIGNED TO CURRENT USER!");
                }
                processDefinition = PbPortlet.getCurrent().bpmModule.getProcessDefinition(task.getProcessDefinitionUUID());
                isNew = Boolean.FALSE;
                dfds = PbPortlet.getCurrent().bpmModule.getProcessDataFields(task.getProcessDefinitionUUID());
                processVars = PbPortlet.getCurrent().bpmModule.getProcessInstanceVariables(task.getProcessInstanceUUID());
                candidates = task.getTaskCandidates();
            } else if (task == null & processDefinition != null) {
                isNew = Boolean.TRUE;
                dfds = PbPortlet.getCurrent().bpmModule.getProcessDataFields(processDefinition.getUUID());
            }
        } catch (Exception ex) {
            ex.printStackTrace();
            return false;
    ...
    public void close() {
        ...
        PbPortlet.getCurrent().portletSession.removeAttribute("PROCESSBASE_SHARED_PROCESSDEFINITION", PortletSession.APPLICATION_SCOPE);
        super.close();
        PbPortlet.getCurrent().portletSession.removeAttribute("PROCESSBASE_PORTLET_CREATED", PortletSession.PORTLET_SCOPE);
