package org.processbase.mobile.server;
...
import org.processbase.mobile.server.template.MobileTask;
...
//            response.sendError(HttpServletResponse.SC_OK);
    ...
    private String getTasks() throws Exception {
        ...
        for (ActivityInstance<TaskInstance> task : tasks) {
            ...
            if (task.getBody().getCreatedDate().after(currentDate)) {
                createdDate = String.format("%1$tH:%1$tM", new Object[]{task.getBody().getCreatedDate()});
            } else {
                createdDate = String.format(new Locale("RU"), "%1$tm %1$tb", new Object[]{task.getBody().getCreatedDate()});
            }
    ...
    private String getTask(String uuid) throws ClassNotFoundException, InstantiationException, IllegalAccessException, ProcessNotFoundException, ActivityNotFoundException, Exception {
        ...
        mobileTask.setTaskInfo(bpmModule, null, activityInstance);
