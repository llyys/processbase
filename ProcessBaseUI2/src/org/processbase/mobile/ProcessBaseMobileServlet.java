...
//            response.sendError(HttpServletResponse.SC_OK);
    ...
    private String getTasks() throws Exception {
        ...
        for (TaskInstance task : tasks) {
            ...
            if (task.getCreatedDate().after(c.getTime())) {
                createdDate = String.format("%1$tH:%1$tM", task.getCreatedDate());
            } else {
                createdDate = String.format(new Locale("RU"), "%1$td %1$tb", task.getCreatedDate());
            }
            result.append("createdDate=\"" + createdDate + "\" ");
            ActivityDefinition activityDefinition = bpmModule.getProcessActivity(task.getProcessDefinitionUUID(), task.getActivityName());
            ...
            result.append("mobile=\"" + pbActivityUi.getIsMobile() + "\" ");
            result.append("isassigned=\"" + (task.isTaskAssigned() ? "T" : "F") + "\">");
            result.append("</task>");
        }
        result.append("</tasks>");
    ...
    private String getTask(String uuid) throws ClassNotFoundException, InstantiationException, IllegalAccessException, ProcessNotFoundException, ActivityNotFoundException, Exception {
        TaskInstance taskInstance = bpmModule.getTaskInstance(new ActivityInstanceUUID(uuid));
        ActivityDefinition activityDefinition = bpmModule.getProcessActivity(taskInstance.getProcessDefinitionUUID(), taskInstance.getActivityName());
        PbActivityUi pbActivityUi = hutil.findPbActivityUi(activityDefinition.getUUID().toString());
        ...
        mobileTask.setTaskInfo(bpmModule, null, taskInstance, currentUser);
