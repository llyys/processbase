...
public class ProcessDashboardPanel_RU extends AnalyticPanel {
    ...
    public void refreshDashboard() {
        try {
            ...
            grid.addComponent(getMeterImage("Ошибки процессов", bpmModule.getProcessInstancesByStatus(InstanceState.ABORTED).size()), 0, 1, 0, 1);

            ArrayList<BarValue> values = new ArrayList<BarValue>();
//            values.add(new BarValue(bpmModule.getProcessInstancesByStatus(InstanceState.INITIAL).size(), "Инициализация", ""));
            values.add(new BarValue(bpmModule.getProcessInstancesByStatus(InstanceState.CANCELLED).size(), "Отменен", ""));
            values.add(new BarValue(bpmModule.getProcessInstancesByStatus(InstanceState.FINISHED).size(), "Завершен", ""));
            values.add(new BarValue(bpmModule.getProcessInstancesByStatus(InstanceState.ABORTED).size(), "Прекращен", ""));
            values.add(new BarValue(bpmModule.getProcessInstancesByStatus(InstanceState.STARTED).size(), "Выполняется", ""));
