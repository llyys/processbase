...
//            }
            ...
            if (!(component instanceof Button) && component instanceof AbstractField) {
                if (value != null) {
...
//                System.out.println(widget.getLabel() + " value = " + widget.isMandatory());
    ...
    private Component getDownload(Widget widget) {
	        ...
	        // if there is no attached document then this button should be disabled mode.
		        ...
		        b.addListener(new Button.ClickListener() {
					...
					public void buttonClick(ClickEvent event) {
						...
						try {
							...
							bytes = ProcessbaseApplication.getCurrent().getBpmModule().getAttachmentValue(processUUID, w.getVariableBound());
...
//            ai = findAttachmentInstance(widget.getInitialValue().getExpression());
	...
	public void submitPage() throws Exception, ProcessNotFoundException,
		...
		{
			showMessage(evalGroovyScript(confirmationMessage), Notification.TYPE_HUMANIZED_MESSAGE);
		}
...
//            System.out.println("expression = " + expression);
        ...
        if (taskInstance != null && !scripts.isEmpty()) {
            groovyScripts = ProcessbaseApplication.getCurrent().getBpmModule().evaluateGroovyExpressions(scripts, taskInstance.getUUID(), context, false, false);
        } else if (taskInstance == null && !scripts.isEmpty()) {
            groovyScripts = ProcessbaseApplication.getCurrent().getBpmModule().evaluateGroovyExpressions(scripts, processDefinition.getUUID(), context, true);
        }
        	 	...
        	 	Map<String, Object> resultGroovyScripts = new HashMap<String, Object>();
	        		 try {
	        			 ...
	        			 if (taskInstance != null && !script.isEmpty()) {
	    	        		 resultGroovyScripts = ProcessbaseApplication.getCurrent().getBpmModule().evaluateGroovyExpressions(scripts, taskInstance.getUUID(), context, false, false);
	    	             } else if (taskInstance == null && !script.isEmpty()) {
	    	            	 resultGroovyScripts = ProcessbaseApplication.getCurrent().getBpmModule().evaluateGroovyExpressions(scripts, processDefinition.getUUID(), context, true);
	    	             }
	        			 script="";
