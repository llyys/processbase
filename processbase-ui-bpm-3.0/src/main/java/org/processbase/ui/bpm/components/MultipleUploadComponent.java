...
public class MultipleUploadComponent extends com.vaadin.ui.VerticalLayout {
	public MultipleUploadComponent(TaskField field, final ProcessInstanceUUID processInstanceUUID, final Widget widget) {
		...
		for (Object object : (Collection) field.getValue()) {
				...
				ImmediateUpload immediateUpload = new ImmediateUpload(processInstanceUUID.toString(), attachment.getName(), field.getVariableBound(), fileName, true, widget.isReadonly(), ProcessbaseApplication.getCurrent().getPbMessages());
	...
	private ImmediateUpload createUploadControl(final ProcessInstanceUUID processInstanceUUID, final Widget widget, final TaskField field) {
		final ImmediateUpload component = new ImmediateUpload(processInstanceUUID.toString(), "Add file", field.getVariableBound(), "", false, widget.isReadonly(),ProcessbaseApplication.getCurrent().getPbMessages());
		component.addListener(new Upload.FinishedListener() {
			public void uploadFinished(FinishedEvent event) {
				try {
					...
					Document document = bpmModule.addDocument(processInstanceUUID, widget.getVariableBound(),event.getFilename(), event.getMIMEType(),component.getFileBody());
